...
- **Last done**: 用户批准浅色样稿；样稿 4 种宽度、详情、历史、状态、键盘与离线检查通过。
- **Next**: 引入 Tailwind CSS、Lucide 并调整正式 React 页面。
- **In flight**: 无
...
### P2 — 依赖与正式页面 [in_progress]
- [ ] 引入 Tailwind CSS 与 Lucide，生产与开发构建接通。
- [ ] 页面按批准样稿调整，保留原有业务交互。
- [ ] 更新构建说明，明确替代旧样式约定。
- **Acceptance**: 无外部 CDN，干净构建可用，图标具名导入，重复样式减少。
- **Verify**: `bun run typecheck && bun run test && bun run build`，开发监听验证 → **Result**: 未运行。
