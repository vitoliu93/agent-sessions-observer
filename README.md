...
- 模型边输出边解析：终端每 15 秒打印一次进度（已收到字数、已出卡片数）；第一版地图生成中，页面每秒刷新，目标和卡片的标题、正文边写边显示、边变长；写到一半的 ID、类型、状态、署名和连线等写完才出现（半截 ID 会撞上别的卡，所以正在写 ID 类字段的那张卡或那条边先不显示）。正式版（模型输出完成后）的校验口径不受草稿影响。已有正式地图时，新一轮分析只在顶栏显示进度，不用草稿替换正式地图。
