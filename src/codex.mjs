// codex.mjs — Codex CLI 会话定位与解析（零依赖）
...
import { readTextSnapshot } from './parse.mjs';
...
export const isCodexFile = file => /(^|\/)rollout-[^/]*\.jsonl$/.test(file) && file.includes('/.codex/');
...
export function codexTitle(sessionId) {
...
export function parseCodexSession(file) {
  ...
  let meta = null;
  const push = (i, d, type, text, blocks) => events.push({ i: events.length, line: i + 1, type, ts: d.timestamp, cwd: meta?.cwd, side: false, text, blocks });
...
export function codexChildren(hostId, hostFile, rollouts = listCodexRollouts()) {
  ...
  const out = [], parents = new Set([hostId]);
  for (let grew = true; grew;) {
    ...
    for (const r of metas) {
      ...
      const agentPath = spawn.agent_path || r.meta.agent_path || '';
      out.push({
