...
export interface Emph { active: boolean; hl: Set<string>; groups: Set<string>; direct: (e: Edge) => boolean }
...
export default function MapView({ app, plan, emph, edges }: { app: AppCtx; plan: Plan; emph: Emph; edges: Edge[] }) {
  ...
  const [layout, setLayout] = useState({ pos: {} as Record<string, Pos>, height: 0, key: '' });
  ...
  useLayoutEffect(() => {
    ...
    if (key !== layout.key) { setLayout({ pos, height: bottom, key }); return; }
    a.stable();
  });

  const place = (id: string, i: number) => {
    const p = layout.pos[id];
    return { left: x(i), top: p ? p.y : 100, width: cw, visibility: p ? undefined : 'hidden' as const };
  };
  const foldEntry = (i: number, list: CardT[]) => {
    ...
    return <button id={'fold-' + i} type="button" style={place('fold-' + i, i)}
      className={`foldentry${emph.active && emph.groups.has('fold-' + i) ? ' hl' : ''}`}
  ...
  return (
    <div className={`wrap${emph.active ? ' focusmode' : ''}`} id="wrap" ref={wrapRef} style={{ height: layout.height }}
      onClick={e => { if (e.target === wrapRef.current || (e.target as Element).id === 'edges') a.background(); }}>
      <Edges ready={ready} W={W} H={Math.max(0, layout.height - 2)} edges={edges} byId={byId} pos={layout.pos}
      ...
      {ready && plan.cols.map((pool, i) => <Fragment key={i}>
        ...
        {pool.map(c => <Card key={c.id} c={c} t={t} style={place(c.id, i)} app={app}
          hl={emph.active && emph.hl.has(c.id)} sel={c.id === s.selectedId} />)}
