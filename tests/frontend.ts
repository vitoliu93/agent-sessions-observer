...
await test('relationship_jumps_to_target_in_focus',async p=>{await mount(p);const target='C14';assert.equal(await p.locator('#c-'+target).count(),0);await p.locator('#c-S1 .dtl').click();await p.locator(`[data-target="${target}"]`).click();assert.equal(await p.locator('#c-'+target).count(),1);assert((await p.locator('#dHead h3').textContent())!.includes(target));});
...
await test('hovered_card_highlights_direct_relations_only',async p=>{await mount(p);const d=fixture();await p.locator('#c-S1').hover();const hl=await p.locator('.card.hl').evaluateAll(ns=>ns.map(n=>(n as HTMLElement).dataset.id!));const direct=new Set(['S1',...d.edges.filter(e=>e.f==='S1'||e.t==='S1').flatMap(e=>[e.f,e.t])]);assert(hl.length>1);assert(hl.every(id=>direct.has(id)),hl.join());});
...
await test('focus_chain_then_back_to_full_map',async p=>{const d=fixture();const all=[...d.goals,...d.cards];const state=await mount(p,d);
  const ids=()=>p.locator('.card:not(.out)').evaluateAll(ns=>ns.map(n=>(n as HTMLElement).dataset.id!).sort());
  const before=await ids(),folds=await p.locator('.foldentry').count();assert(folds>0);
  const rid=(await p.locator('.card.risk').first().getAttribute('data-id'))!,n=rid.slice(1);const expect=[...chain(all,d.edges,rid)].sort();assert.deepEqual(expect,['C'+n,'G'+n,'GOAL','K'+n,rid,'S'+(+n%2+1),'V'+n].sort());
  await p.locator('#c-'+rid).click();assert(await p.locator('.card.out').count()>0,'链路外的卡先淡出');
  await p.waitForTimeout(700);assert.deepEqual(await ids(),expect);assert.equal(await p.locator('.foldentry').count(),0);
  assert.match((await p.locator('#scope').textContent())!,/聚焦.*7 张卡/);
  assert.match(await p.locator('#c-'+rid).evaluate(n=>getComputedStyle(n).transition),/top/);
  const tops=await p.evaluate(()=>[...document.querySelectorAll<HTMLElement>('.card')].map(n=>n.offsetTop));assert(tops.every(y=>y<1080));
